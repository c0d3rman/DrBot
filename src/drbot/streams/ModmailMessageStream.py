...
class ModmailMessageStream(TimeGuardedStream[ModmailMessage]):
    ...
    def get_items_raw(self) -> Iterable[ModmailMessage]:
        ...
        conversation_i = 0
        for conversation in reddit.sub.modmail.conversations(limit=10, sort="recent"):
            message_i = 0
            conversation_i += 1
    ...
    def get_latest_item(self) -> ModmailMessage | None:
        latest_conversation = next(reddit.sub.modmail.conversations(limit=1, sort="recent"), None)
